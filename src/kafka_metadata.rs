...
#[derive(Debug)]
pub struct MetadataBind {
    config: HashMap<String, String>,
}

pub struct MetadataInit {
    state: Mutex<Option<MetadataState>>,
}

struct MetadataState {
    brokers: Vec<BrokerRow>,
    topics: Vec<TopicRow>,
    emitted: bool,
}
...
impl VTab for KafkaMetadata {
    ...
    fn func(
        ...
        let mut guard = init
            .state
            .lock()
            .map_err(|_| "Kafka metadata state mutex was poisoned")?;

        if guard.is_none() {
            *guard = Some(MetadataState::fetch(&bind.config)?);
        }
...
fn write_metadata(
    ...
    // A metadata result has exactly one output row. The LIST children can be
    // much larger than a normal DuckDB data chunk, so reserve their child
    // storage explicitly before taking nested vector views.
    {
...

        {
            ...
            for (slot, broker) in values.iter_mut().zip(brokers) {
                *slot = broker.id;
            }
        }

        {
            ...
            for (index, broker) in brokers.iter().enumerate() {
                hosts.insert(index, broker.host.as_str());
            }
        }
        ...
        // The outer vector has one row; its child vector has `brokers.len()`
        // elements. `try_set_len` applies to the LIST child storage, not the
        // number of rows in the parent chunk.
...

    {
...

        {
            ...
            let values = unsafe {
                partition_ids.as_mut_slice_with_len::<i32>(total_partitions)
            };
...

        {
            ...
            let values = unsafe {
                partition_leaders.as_mut_slice_with_len::<i32>(total_partitions)
            };
            let mut index = 0usize;
            for topic in topics {
                for partition in &topic.partitions {
        ...
        for (index, topic) in topics.iter().enumerate() {
            names.insert(index, topic.name.as_str());
            match &topic.error {
                Some(error) => errors.insert(index, error.as_str()),
                None => errors.set_null(index),
            }

            let length = topic.partitions.len();
            partition_lists.set_entry(index, partition_offset, length);
            partition_offset += length;
        }

        // `partition_lists` has one LIST entry per topic, while its child
        // storage contains all partitions flattened across topics.
        partition_lists.try_set_len(total_partitions)?;

        // As above, the outer topics LIST contains one parent row and all topic
        // structs live in its child storage.
...
mod tests {
    ...
    fn test_chunk() -> DataChunkHandle {
        ...
        DataChunkHandle::new(&[
            LogicalTypeHandle::list(&broker_type),
            LogicalTypeHandle::list(&topic_type),
        ])
    }

    #[test]
    fn metadata_vector_layout_handles_multiple_brokers_and_nested_topics() {
        let brokers = vec![
            BrokerRow {
                id: 1,
        ...
        let topics = vec![
            TopicRow {
                ...
                error: None,
                partitions: vec![PartitionRow { id: 0, leader: 1 }],
            },
            TopicRow {
                name: "topic-b".to_owned(),
                error: None,
                partitions: vec![
                    PartitionRow { id: 0, leader: 2 },
                    PartitionRow { id: 1, leader: 3 },
                ],
            },
        ];

        let mut chunk = test_chunk();
        write_metadata(&mut chunk, &brokers, &topics).unwrap();

        assert_eq!(chunk.list_vector(0).get_entry(0), (0, 3));
        assert_eq!(chunk.list_vector(1).get_entry(0), (0, 2));
        chunk.set_len(1);
    }

    #[test]
    fn metadata_vector_layout_handles_metadata_larger_than_standard_chunk() {
        let brokers = vec![BrokerRow {
            id: 1,
            host: "broker-1".to_owned(),
            port: 9092,
        }];
        let topics: Vec<_> = (0..4096)
            .map(|id| TopicRow {
        ...
        assert_eq!(chunk.list_vector(0).get_entry(0), (0, 1));
        assert_eq!(chunk.list_vector(1).get_entry(0), (0, 4096));
        chunk.set_len(1);
    }
}
